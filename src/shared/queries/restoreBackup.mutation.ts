...
const parseBackup = (json: string): BackupFile => {
  ...
  try {
    ...
    throw new Error('این فایل JSON معتبر نیست. مطمئن شو همون فایلی رو انتخاب کردی که از «بکاپ» گرفته بودی.')
  }

  if (typeof parsed !== 'object' || parsed === null) {
    throw new Error('محتوای فایل خالی یا خراب است.')
  }

  const candidate = parsed as Partial<BackupFile>

  if (candidate.app !== 'daramadname') {
    throw new Error('این فایل مال درآمدنامه نیست. فایل بکاپ یه ابزار دیگه رو انتخاب کردی؟')
  }
  if (candidate.version !== 1) {
    throw new Error('نسخه‌ی این فایل بکاپ پشتیبانی نمی‌شه. با نسخه‌ی جدیدتر برنامه ساخته شده.')
  }
  if (!Array.isArray(candidate.receipts) || !Array.isArray(candidate.clients)) {
    throw new Error('ساختار فایل بکاپ ناقصه؛ فهرست دریافتی‌ها یا مشتری‌ها توش نیست.')
  }
  ...
  for (const receipt of candidate.receipts as Receipt[]) {
    if (typeof receipt.id !== 'string' || typeof receipt.occurredAt !== 'string' || typeof receipt.amountToman !== 'number') {
      throw new Error('یکی از دریافتی‌های داخل فایل ناقصه و قابل بازیابی نیست.')
    }
  }
  for (const client of candidate.clients as Client[]) {
    if (typeof client.id !== 'string' || typeof client.name !== 'string') {
      throw new Error('یکی از مشتری‌های داخل فایل ناقصه و قابل بازیابی نیست.')
    }
  }
